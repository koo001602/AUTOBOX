...
def read_image_with_korean_path(image_path: str):
    """한글 경로의 이미지를 읽습니다."""
    # numpy로 바이트 읽기 후 cv2로 디코딩 (한글 경로 지원)
...
def ocr_image(image_path: str, reader, languages: list = ['ko', 'en']) -> dict:
    """
    이미지에서 텍스트를 추출합니다.
    ...
    Args:
        ...
        languages: 인식할 언어 리스트 (기본값: 한글, 영어)
...
def ocr_folder(folder_path: str, languages: list = ['ko', 'en']) -> list:
    """
    ...
    for file in folder.iterdir():
        if file.suffix.lower() in image_extensions:
            ...
            result = ocr_image(str(file), reader, languages)
...
def main():
    ...
    output_file = Path(__file__).parent / "ocr_result.txt"
