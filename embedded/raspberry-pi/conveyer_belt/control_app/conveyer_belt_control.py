...
class ConveyerController:
    ...
    def belt_run(self):
        ...
        self.move_motor_async(5, Raspi_MotorHAT.BACKWARD)

    def belt_stop(self):
        self.stop_event.set()


    def move_servos_smooth(self,ch1_start, ch1_end, ch2_start, ch2_end, speed=0.005):
    
        steps = 200  # 이동을 나눌 단계 (높을수록 부드러움)

        for i in range(steps + 1):
            # 현재 위치 계산 (선형 보간)
            pos1 = int(ch1_start + (ch1_end - ch1_start) * (i / steps))
            pos2 = int(ch2_start + (ch2_end - ch2_start) * (i / steps))
    ...
    def servo_push(self):
        self.move_servos_smooth(600, 180, 180, 600)


    def servo_pull(self):
        self.move_servos_smooth(180, 600, 600, 180)
